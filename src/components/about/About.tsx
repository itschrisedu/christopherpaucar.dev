...
export default function About() {
  ...
  const stats = [
    ...
    { value: "20+", label: locale === "en" ? "Projects Delivered" : "Proyectos entregados" },
    { value: "15+", label: locale === "en" ? "Happy Clients" : "Clientes satisfechos" },
    { value: "99%", label: locale === "en" ? "On-Time Delivery" : "Entrega a tiempo" },
  ];

  return (
    <section id="about" ref={ref} className="relative py-32 sm:py-40 bg-white dark:bg-[#0b0b0b] overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-6 sm:px-8 lg:px-16 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-20 items-start">
          {/* Photo + Stats */}
          <motion.div initial={{ opacity: 0, x: -30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8, ease: EASE }} className="space-y-7">
            ...
            <div className="grid grid-cols-2 gap-4">
              {stats.map((s, i) => (
                <motion.div key={s.label} initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.3 + i * 0.1, ease: EASE }}
                  className="p-6 rounded-2xl bg-surface dark:bg-[#151515] border border-border dark:border-white/8
                             hover:border-accent/25 transition-all duration-500 hover:-translate-y-0.5"
                >
                  <p className="text-[2rem] font-bold text-accent tracking-tight">{s.value}</p>
                  <p className="mt-1.5 text-[12px] font-semibold text-secondary uppercase tracking-wider">{s.label}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
          ...
          {/* Text */}
          <motion.div initial={{ opacity: 0, x: 30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8, delay: 0.15, ease: EASE }} className="space-y-8 lg:pt-6">
            <div>
              <span className="inline-block text-[11px] font-mono font-bold uppercase tracking-[0.3em] text-accent mb-5">{locale === "en" ? "About" : "Acerca"}</span>
            ...
            <div>
              <h3 className="text-[11px] font-mono font-bold uppercase tracking-[0.3em] text-accent mb-7">{locale === "en" ? "Experience" : "Experiencia"}</h3>
              <div className="p-7 rounded-2xl bg-surface dark:bg-[#151515] border border-border dark:border-white/8 hover:border-accent/25 transition-all duration-500">
                ...
                <p className="mt-4 text-[14px] text-secondary leading-[1.7]">{locale === "en" ? "Building scalable web apps for clients across industries. Focus on conversion and performance." : "Construyendo apps web escalables para clientes de diversas industrias. Enfoque en conversión y rendimiento."}</p>
              </div>
            </div>
          </motion.div>
        </div>
